 ...
 * @access Public
 */
router.get('/suggestions', suggestionLimiter, async (req: Request, res: Response) => {
  try {
    ...
    if (USE_MOCK_ELASTICSEARCH) {
      // 使用模擬服務
      suggestions = await mockElasticsearchService.getSearchSuggestions(query, limitNum);
    } else {
      // 檢查 Elasticsearch 連接
      if (!elasticsearchService.isConnected()) {
        return res.json({
      ...
      suggestions = await petSearchService.getSearchSuggestions(query, limitNum);
    }
