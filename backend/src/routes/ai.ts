...
router.post(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      // 執行 AI 分析
      const analysisResult = await AIService.analyzeImageWithVision(
        req.file.buffer,
      );

      // 生成搜尋建議
      const searchSuggestions =
        AIService.generateSearchSuggestions(analysisResult);
...
router.post(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      // 優化圖像
      const { buffer: optimizedBuffer, metadata } =
        await AIService.optimizeImage(req.file.buffer, options);
      ...
      res.json({
        ...
        data: {
          ...
          metadata: {
            ...
            compressionRatio:
              (
                ((req.file.size - optimizedBuffer.length) / req.file.size) *
                100
              ).toFixed(2) + "%",
          },
        },
...
router.post(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      // 裁剪圖像
      const croppedBuffer = await AIService.cropImage(
        req.file.buffer,
        cropOptions,
      );
...
router.post(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      // 提取上傳圖像的特徵
      const queryFeatures = await AIService.extractImageFeatures(
        req.file.buffer,
      );
