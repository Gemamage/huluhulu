...
export const petSchema = z.object({
  ...
  age: z.number().int().min(0, '年齡不能為負數').max(30, '年齡不能超過 30 歲').optional(),
  color: z.string().trim().max(50, '顏色描述不能超過 50 個字符').optional(),
  ...
  medicalConditions: z.string().trim().max(500, '醫療狀況描述不能超過 500 個字符').optional(),
  specialMarks: z.string().trim().max(500, '特殊標記描述不能超過 500 個字符').optional(),
  personality: z.string().trim().max(500, '個性描述不能超過 500 個字符').optional(),
});
