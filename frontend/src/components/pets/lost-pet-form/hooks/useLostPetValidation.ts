...
export function useLostPetValidation() {
  const validateForm = useCallback(
    (formData: LostPetFormData): { isValid: boolean; errors: FormErrors } => {
      ...
      // 走失地點驗證
      if (
        !formData.lostLocation.city ||
        !formData.lostLocation.district ||
        !formData.lostLocation.address.trim()
      ) {
      ...
      if (
        ...
        !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(formData.ownerContact.email)
      ) {
      ...
      if (
        ...
        (formData.weight <= 0 || formData.weight > 200)
      ) {
      ...
      if (
        formData.lostTime &&
        !/^([01]?[0-9]|2[0-3]):[0-5][0-9]$/.test(formData.lostTime)
      ) {
