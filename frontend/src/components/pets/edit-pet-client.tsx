...
export default function EditPetClient({ petId }: EditPetClientProps) {
  ...
  const fetchPet = async () => {
    try {
      ...
      // 檢查是否為當前用戶的寵物
      const petOwnerId =
        typeof petData.owner === 'object' ? petData.owner._id : petData.owner;
  ...
  const handleSubmit = async (formData: PetFormData) => {
    ...
    try {
      ...
      const updateData: any = {
        ...
        lastSeenLocation:
          formData.lastSeenLocation.latitude &&
          formData.lastSeenLocation.longitude
            ? {
