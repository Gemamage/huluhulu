...
interface FoundPetFormData {
  // 基本資訊（選填或未知選項）
  name?: string;
  type: 'dog' | 'cat' | 'bird' | 'rabbit' | 'hamster' | 'fish' | 'reptile' | 'other' | 'unknown';
  breed: string[]; // 改為多選標籤
...
export function FoundPetForm({ initialData, onSubmit, isLoading = false }: FoundPetFormProps) {
  ...
  const validateForm = (): boolean => {
    ...
    if (formData.finderContact.email && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(formData.finderContact.email)) {
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            />
            {errors.foundDate && <p className="text-sm text-red-500 mt-1">{errors.foundDate}</p>}
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <Input
              ...
              onChange={(e) => handleInputChange('finderContact.name', e.target.value)}
              placeholder="例如：陳小姐、熱心的王先生"
              className={errors['finderContact.name'] ? 'border-red-500' : ''}
            />
            {errors['finderContact.name'] && <p className="text-sm text-red-500 mt-1">{errors['finderContact.name']}</p>}
          </div>
          ...
          <div>
            ...
            <Input
              ...
              onChange={(e) => handleInputChange('finderContact.phone', e.target.value)}
              placeholder="請填寫可聯絡的電話號碼"
              className={errors['finderContact.phone'] ? 'border-red-500' : ''}
            />
            {errors['finderContact.phone'] && <p className="text-sm text-red-500 mt-1">{errors['finderContact.phone']}</p>}
          </div>
          ...
          <div>
            ...
            <Input
              ...
              onChange={(e) => handleInputChange('finderContact.email', e.target.value)}
              placeholder="選填，方便聯絡"
            />
          </div>
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) => handleInputChange('finderContact.preferredContact', e.target.value)}
              className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
            >
